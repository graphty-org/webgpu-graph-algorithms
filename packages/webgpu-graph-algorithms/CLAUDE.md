...
| The host lane's adapters (hosts.yml, first run 2026-09-16)                                                           | macos-latest: Dawn-node picks `vendor=apple architecture= device=apple-paravirtual-device` ("Metal driver on macOS 26.6.2"), subgroups 32-32, maxBufferSize 3.75 GB -- a real Metal device in the VM; windows-latest: `adapter=Microsoft Basic Render Driver` selects `vendor=microsoft architecture=warp`, subgroups 4-128 (min != max, the Intel-Xe-like case the caps tables fake), NO `subgroups` feature, maxBufferSize 2 GiB. WARP is a CPU rasterizer, so `isSoftwareAdapter` / `isSoftwareInfo` classify `architecture === "warp"` as software (gpuScale 1/50, `rejectSoftware` honours it)                                                                            | the hosts.yml adapter reports and `[gpu] adapter` lines of run 35118525921                                                                                                                       |
| Dawn on Metal and a buffer destroyed while its mapAsync is pending                                                   | the macOS node run died with tinypool's `Channel closed` (the worker process vanished) in `test/memory/readback.test.ts` right at the test that calls `destroyAll()` with a read pending; Vulkan backends reject the map and go on. `Readback.destroyAll()` therefore defers the destruction of a slot with a pending map to the moment the map settles (the read still rejects `E_DISPOSED`). Whether this is the whole story is what the second hosts.yml run shows                                                                                                                                                                                                          | run 35118525921 (807 tests passed before the crash, 0 failed); `src/memory/readback.ts`                                                                                                          |
